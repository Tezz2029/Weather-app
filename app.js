const key = "8cd80d49e5c0dcb0f5b9437939f63e0c";
...
let weather = 
{
    ...
    fetchCountryId: (latitude, longitude) =>{
        fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${key}`).then((response)=> response.json())
        .then((data)=>{
            const {name} = data;
    ...
    fetchLatLon:(city)=> {
        ...
        .then((data)=>{
            ...
            const {country} = data.sys;
            weather.displayWeather(lat,lon,country,city);
        });
    ...
    fetchOnecallWeather:function(latitude, longitude, country, city)
    {
        ...
        .then((response)=> response.json()).then((data)=>this.displayWeather(data,country,city));

    },

    displayWeather:function(data,country,city){
        const {icon,description} = data.current.weather[0];
        const {humidity,temp,wind_speed} = data.current;
        ...
        document.querySelector("#h-value").innerHTML = `${wind_speed}`;
        document.body.style.backgroundImage = `url('https://source.unsplash.com/1920x1080/?${city}')`;
        document.querySelector("#temp1").innerHTML = `${morn}`;
        document.querySelector("#temp2").innerHTML = `${day}`;
        document.querySelector("#temp3").innerHTML = `${eve}`;
        document.querySelector("#temp4").innerHTML = `${night}`;
    },
    ...
    LocalDay:function()
    {
        ...
        document.querySelector(".day-dayname").innerHTML = day;
        var dd = d.getDate();
        var mm = new Array(12);
        mm[1] = "January";
        mm[2] = "February";
        ...
        document.querySelector(".day-day").innerHTML = dd+" "+month+", "+yyyy;
...
//Event Listner for enter and click
document.querySelector(".search").addEventListener("keypress", function(random) 
{
    if (random.key == "Enter") {
        weather.search();
    }
